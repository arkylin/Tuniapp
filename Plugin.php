 ...
 * 
 * @package Tuniapp
 * @author <a href="https://www.xyz.blue">Arkylin</a>
 * @version 0.0.1
 * @link https://www.xyz.blue
 */
class Tuniapp_Plugin implements Typecho_Plugin_Interface
{

    public static function activate()
    {
        Typecho_Plugin::factory('Widget_Feedback')->finishComment = array('Tuniapp_Plugin', 'sendFeedback');
        Helper::addRoute("uniapi", "/uniapi/[type]", "Tuniapp_Action", 'action');
        $db = Typecho_Db::get();
        $prefix = $db->getPrefix();
        ...
        //创建用户数据库
        $scripts = file_get_contents('usr/plugins/Tuniapp/sql/' . $type . '.sql');
    ...
    public static function config(Typecho_Widget_Helper_Form $form)
    {
        ...
        $crossCid = new Typecho_Widget_Helper_Form_Element_Text('crossCid', NULL, '1', _t('时光机页面CID'),  _t('小程序时光机页面显示内容'));
        $form->addInput($crossCid);
    ...
    public static function sendFeedback($comment)
    {
        ...
        $appId = Typecho_Widget::widget('Widget_Options')->plugin('Tuniapp')->appId;
        $appSecret = Typecho_Widget::widget('Widget_Options')->plugin('Tuniapp')->appSecret;
        ...
        if (preg_match('/(.*)@wx\.com/', $original['mail'], $matches)) {
            ...
            $info = curl_exec($ch);
            $row = $db->fetchRow($db->select('formid')->from('table.Tuniapp')->where('openid = ?', $openid));
            $db->query($db->update('table.Tuniapp')->rows(array('formid' => (int) $row['formid'] - 1))->where('openid = ?', $openid));
            return $info;
        }
    }
}
